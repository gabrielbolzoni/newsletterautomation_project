...
def email_reader():
    ...
    # time.
    if os.path.exists("/opt/airflow/config/credentials/token.json"):
        creds = Credentials.from_authorized_user_file("/opt/airflow/config/credentials/token.json", SCOPES)
    # If there are no (valid) credentials available, let the user log in.
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file("/opt/airflow/config/credentials/credentials.json", SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open("/opt/airflow/config/credentials/token.json", "w") as token:
